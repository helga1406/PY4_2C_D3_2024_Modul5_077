...
class LogController {
  ...
  LogController({required this.username, this.userRole = 'Anggota'}) {
  ...
  Future<void> _syncPendingData() async {
    await LogHelper.writeLog("INTERNET AKTIF: Memulai sinkronisasi data lokal ke Cloud...", level: 2);
    ...
    for (var log in localLogs) {
      try {
        ...
        await LogHelper.writeLog("ERROR: Sinkronisasi gagal untuk log ${log.id}", level: 1);
      }
    }
    await LogHelper.writeLog("SYNC SELESAI: Semua data disinkronisasi ke MongoDB.", level: 2);
  }
  ...
  Future<void> addLog(String title, String desc, String category) async {
    if (!AccessControlService.canPerform(userRole, AccessControlService.actionCreate)) {
      await LogHelper.writeLog("SECURITY BREACH: Unauthorized create attempt by $username", source: "LogController", level: 1);
      return; 
    }
  ...
  Future<void> updateLog(LogModel updatedLog) async {
    ...
    if (!AccessControlService.canPerform(userRole, AccessControlService.actionUpdate, isOwner: isOwner)) {
      await LogHelper.writeLog("SECURITY BREACH: Unauthorized update attempt by $username", source: "LogController", level: 1);
      return; 
    }
  ...
  Future<void> removeLog(LogModel log) async {
    ...
    if (!AccessControlService.canPerform(userRole, AccessControlService.actionDelete, isOwner: isOwner)) {
      await LogHelper.writeLog("SECURITY BREACH: Unauthorized delete attempt by $username", source: "LogController", level: 1);
      return; 
    }
    ...
    if (log.id != null) {
      ...
      try {
        ...
        await LogHelper.writeLog("WARNING: Hapus lokal berhasil, gagal di Cloud", level: 1);
      }
    }
  }
}